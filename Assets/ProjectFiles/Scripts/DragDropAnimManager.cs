...
public class DragDropAnimManager : MonoBehaviour
{
    ...
    public class PageEntry
    {
        ...
        [Tooltip("If TRUE, when this page's animation finishes (or there's no animation to play), the Next button gets unlocked automatically via PageNavigationController.RequestNavigationUnlock(). If FALSE, that call is skipped — Next stays locked until something else unlocks it.")]
    ...
    private void OnSnapped(int pageIndex, PageEntry entry)
    {
        ...
        if (entry.animation != null && entry.animation.IsValid)
        {
            ...
            StartCoroutine(entry.animation.Play(this, () =>
            {
                ...
                TryUnlockNavigation(pageIndex, entry);
            }));
        }
        else
        {
            Debug.LogWarning($"[DragDropAnimManager] Page {pageIndex}: no valid AnimationSource.");
            TryUnlockNavigation(pageIndex, entry);
        }
    }

    // -----------------------------------------------------------------
    // Single gate for the Next-button unlock. Called after animation
    // completes (or immediately if there's no animation). Only actually
    // calls PageNavigationController.RequestNavigationUnlock() when this
    // entry's "unlockNavigationOnComplete" bool is ticked ON in the
    // Inspector; otherwise it's skipped entirely and Next stays locked.
    // -----------------------------------------------------------------
    private void TryUnlockNavigation(int pageIndex, PageEntry entry)
    {
        if (entry.unlockNavigationOnComplete)
        {
            Debug.Log($"[DragDrop] Page {pageIndex} — unlockNavigationOnComplete = true, requesting navigation unlock.");
            PageNavigationController.RequestNavigationUnlock();
        }
        else
        {
            Debug.Log($"[DragDrop] Page {pageIndex} — unlockNavigationOnComplete = false, NOT unlocking Next.");
        }
    }

    ...
    private void TryBeginDrag(PageEntry entry)
{
    ...
    private void EvaluateDrop(int pageIndex, PageEntry entry)
    {
        ...
        Debug.Log($"[DragSnap] Page {pageIndex} release check — overlapping={overlapping}, angle={angle:F2} (max {entry.snapAngle})");

        if (overlapping && angle <= entry.snapAngle)
        {
            Debug.Log($"[DragSnap] Page {pageIndex} — SNAP PASSED.");
        ...
        {
            Debug.Log($"[DragSnap] Page {pageIndex} — snap FAILED, out of tolerance. Returning to start position.");
